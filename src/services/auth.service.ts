...
export async function signUpService({ username, password }: { username: string; password: string }) {
...
export async function signInService({ username, password }: { username: string, password: string }) {
...
export async function refreshTokenService(oldRefreshToken: string) {
  ...
  const decoded = jwt.verify(oldRefreshToken, REFRESH_TOKEN_SECRET) as { userId: string };
